...
namespace {
constexpr int kCodeCooldownSec = 60;
}
...
PairingScreen::PairingScreen(lazytv::AppContainer* container, QWidget* parent)
    ...
    connect(m_key, &QLineEdit::textChanged, this, [this](const QString& t) {
        ...
        if (digits != m_key->text()) m_key->setText(digits);
    });
    ...
    m_infoLabel  = new QLabel(this);  m_infoLabel->setWordWrap(true);
    m_errorLabel = new QLabel(this);  m_errorLabel->setWordWrap(true);
    root->addWidget(m_infoLabel);
    root->addWidget(m_errorLabel);

    if (m_container->store().ip().has_value() &&
        !m_container->store().session().has_value()) {
        setInfo("Сессия устарела. Введите код заново.");
    }
...
void PairingScreen::setLoading(bool v) {
    m_reqBtn->setEnabled(!v && m_reqCooldown == nullptr);
...
void PairingScreen::requestKey() {
    ...
    if (ip.isEmpty()) { setError("Введите IP телевизора"); return; }
    ...
    connect(m_activeClient, &lazytv::Client::pairingKeyResult, this,
        ...
        setInfo("Смотрите код на экране ТВ. Не запрашивайте повторно — "
                "старый код перестанет работать.");

        m_reqBtn->setEnabled(false);
        m_reqCooldown = new QTimer(this);
        m_reqCooldown->setSingleShot(true);
        connect(m_reqCooldown, &QTimer::timeout, this, [this]() {
            m_reqBtn->setEnabled(true);
            m_reqCooldown->deleteLater();
            m_reqCooldown = nullptr;
        });
        m_reqCooldown->start(kCodeCooldownSec * 1000);
    });
...
void PairingScreen::confirm() {
    ...
    if (ip.isEmpty())      { setError("Введите IP"); return; }
    if (key.length() != 6) { setError("Код — 6 цифр"); return; }
    ...
    connect(m_activeClient, &lazytv::Client::pairingConfirmResult, this,
        ...
        QString msg;
        if (r.httpStatus != 200) {
            msg = QStringLiteral("ТВ ответил HTTP %1").arg(r.httpStatus);
        } else if (r.roapError != 0 && r.roapError != 200) {
            msg = QStringLiteral("Код отклонён (ROAPError %1").arg(r.roapError);
            if (!r.detail.isEmpty()) msg += QStringLiteral(": %1").arg(r.detail);
            msg += QLatin1Char(')');
        } else {
            msg = QStringLiteral("ТВ не вернул session");
        }
        setError(msg);
    });
